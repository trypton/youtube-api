...
class YouTubeApi {
  ...
  at(...path) {
    return this.url.replace(/\/+$/, '') + (path.length ? '/' : '') + path.map(at => encodeURIComponent(at)).join('/');
  }
  /**
  ...
  async makeApiRequest({
    ...
    const signal = isAbortControllerSupported && (this.abortController = new AbortController()) && this.abortController.signal; // Authorization with access token
    ...
    if (method === 'GET') {
      const query = YouTubeApi.makeQueryString({ ...this.options,
        ...params
      });
    ...
    const body = method === 'POST' ? JSON.stringify({ ...this.options,
      ...params
    }) : null; // Send request
    ...
    try {
      response = await Promise.race([(0, _nodeFetch.default)(url, {
        method,
        headers,
        body,
        signal
    ...
    if (!response.ok) {
      ...
      if (data && data.error) {
        if (data.error.error_description) {
          throw new _YouTubeError.YouTubeError(data.error.error_description);
        } else {
          throw new _YouTubeError.YouTubeApiError(data.error);
        }
      }

      throw new _YouTubeError.YouTubeError(data || response.statusText || 'Status: ' + response.status);
    }
